...
{
	internal static class DataTemplateConverterPluginHelper
	{
		private static readonly Lazy<Dictionary<string, Lazy<IDataTemplateConverterPlugin, DataTemplateConverterPluginExportMetadataView>>>
			LazyPlugins = new Lazy<Dictionary<string, Lazy<IDataTemplateConverterPlugin, DataTemplateConverterPluginExportMetadataView>>>(InitializePluginsFactory);

		public static IDataTemplateConverterPlugin GetPlugin(string type) =>
			LazyPlugins.Value.TryGetValue(type, out var lazyPlugin) ? lazyPlugin.Value : null;

		private static Dictionary<string, Lazy<IDataTemplateConverterPlugin, DataTemplateConverterPluginExportMetadataView>> InitializePluginsFactory()
		{
